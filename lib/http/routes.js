...
import { fetchViaHttp, gitLsRemote, probeUrl, systemProxy } from '../services/probe.js';
...
import { isDshPlugin, isEntryLoaded } from '../services/loader.js';
import { loadSettings, saveSettings, resetSettings } from '../services/settings.js';
import { appendLog, clearLog, readLog, logFilePath, defaultLogFilePath, customLogFile } from '../services/log.js';
/**
...
function catalogCacheFile(profile, key) {
...
function readCatalogCache(file, maxAgeMs) {
    try {
...
function writeCatalogCache(file, body) {
    try {
...
function installedRepoOf(profile, name) {
    try {
...
function installedByRepo(installed, target) {
    const repo = githubRepoOf(target);
    if (repo === null)
        return null;
    const needle = repo.toLowerCase();
    for (const [name, spec] of Object.entries(installed)) {
        if (githubRepoOf(spec)?.toLowerCase() === needle)
            return name;
    }
    return null;
}
 ...
 */
export function mountPluginHubRoutes(webServer, profile, loader) {
    ...
    const disposers = [
        ...
        webServer.register({
            ...
            handler: async (request, response) => {
                ...
                try {
                    ...
                    for (const key of ['checkUpdatesOnStart', 'enableNpmInstall', 'enableGitInstall', 'enableDshInstall', 'npmRegistry', 'proxy', 'logPath']) {
        ...
        webServer.register({
            ...
            handler: async (request, response) => {
                ...
                const send = (line) => { response.write(`${JSON.stringify(line)}\n`); };
                ...
                const probe = (url, proxy) => probeUrl(url, proxy ?? effectiveProxy, 6000);
        ...
        webServer.register({
            ...
            handler: async (request, response) => {
                ...
                try {
                    ...
                    let mode = body !== null && typeof body === 'object' && body.mode === 'update'
                        ? 'update'
                        : 'install';
                    ...
                    const rawRepo = typeof body === 'object' && body !== null && typeof body.repo === 'string'
                        ? installTargetOf(body.repo)
                        : '';
                    // 前端决策的展示用仓库名（owner/repo）：npm 安装时 body.repo 是包名，display 用于
                    // 待重启行 / 任务恢复时展示仓库名，保证用户无感知（安装通道变化不改变所见目标）
                    const displayRepo = typeof body === 'object' && body !== null && typeof body.display === 'string'
                        ? body.display.trim()
                        : rawRepo;
                    // 请求来源：目录插件安装（catalog）走目录白名单；命令行安装（custom）按通道受安全开关控制
                    const source = typeof body === 'object' && body !== null && typeof body.source === 'string'
                        ? body.source
                        : '';
                    // 自定义安装入口渠道（客户端三张卡片：NPM 包 / GitHub 源码 / DSH 命令）：
                    // 客户端显式上报，服务端日志/报错据此精准溯源「从哪个入口发起」；目录插件安装不传。
                    const installChannel = (() => {
                        const raw = body !== null && typeof body === 'object' ? body.installChannel : undefined;
                        return raw === 'git' || raw === 'dsh' || raw === 'npm' ? raw : undefined;
                    })();
                    const settings = loadSettings(profile);
                    // 界面语言（客户端随请求携带）：错误消息按用户当前语言提示（中文界面给中文，英文界面给英文）
                    const lang = typeof body === 'object' && body !== null && body.lang === 'en'
                        ? 'en'
                        : 'zh';
                    ...
                    // 保证直接调 API 传完整命令也能识别。
                    const bodyGlobalNpm = Array.isArray(body.globalNpm)
                        ? body.globalNpm.filter((x) => typeof x === 'string' && x !== '')
                        : [];
                    let globalNpm = bodyGlobalNpm.length > 0 ? bodyGlobalNpm : (globalNpmPackagesOf(rawRepo) ?? []);
        ...
        webServer.register({
            ...
            handler: async (request, response) => {
                ...
                try {
                    ...
                    const repo = typeof body === 'object' && body !== null && typeof body.repo === 'string'
                        ? body.repo
                        : '';
        ...
        webServer.register({
            ...
            handler: async (request, response) => {
                ...
                try {
                    const body = await readJsonBody(request);
                    const id = typeof body === 'object' && body !== null && typeof body.id === 'number'
                        ? body.id
                        : NaN;
        ...
        webServer.register({
            ...
            handler: async (request, response) => {
                ...
                try {
                    ...
                    const version = body !== null && typeof body.version === 'string'
                        ? body.version
                        : null;
                    if (version === null) {
                        ...
                        const updatedAt = body !== null && typeof body.updatedAt === 'string'
                            ? body.updatedAt
                            : '';
            ...
            handler: (request, response) => {
                ...
                for (const name of Object.keys(installed)) {
                    ...
                    if (isEntryLoaded(loader, name))
                        loaded.push(name);
                    if (isDshPlugin(profile, name))
                        dshCapable.push(name);
                }
