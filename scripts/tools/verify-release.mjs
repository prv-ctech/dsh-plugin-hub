...
if (pkg.name !== expectedName || pkg.publishConfig?.registry !== expectedRegistry || !semver.test(pkg.version)) {
  throw new Error(`Invalid fork package metadata: ${pkg.name}@${pkg.version}`)
}
if (process.env.GITHUB_ACTIONS === 'true' && process.env.GITHUB_REF_NAME !== `v${pkg.version}`) {
