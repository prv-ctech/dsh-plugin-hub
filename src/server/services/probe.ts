...
async function requestUrl(url: string, proxy: string, timeoutMs: number): Promise<{ status: number; body: string }> {
...
export async function probeUrl(url: string, proxy: string, timeoutMs: number): Promise<ProbeResult> {
  let target: URL
  try { target = new URL(url) } catch { return probeFail() }
  if (target.protocol !== 'http:' && target.protocol !== 'https:') return probeFail()
